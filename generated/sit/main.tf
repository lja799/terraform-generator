resource "azurerm_network_security_rule" "agw_rule" {
  ...
  source_port_range            = "*"
  destination_address_prefixes = concat(local.ipam.networks.test-sit.appgw)
  destination_port_ranges      = concat([local.ports.http, local.ports.https])
  resource_group_name          = "network"
  network_security_group_name  = "sit-australiaeast-infrastructure"
}

resource "azurerm_network_security_rule" "management_sit" {
  name      = "Management-${title(sit)}"
  priority  = 501
  direction = "Inbound"
  access    = "Allow"
  protocol  = "Tcp"
  ...
  source_port_range            = "*"
  destination_address_prefixes = local.ipam.networks.sit.nodes
...
resource "azurerm_network_security_rule" "staff_to_storage_file" {
  ...
  source_port_range            = "*"
  destination_address_prefixes = concat(local.ipam.networks.sit.storage-file-ip)

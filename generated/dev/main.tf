resource "azurerm_network_security_rule" "agw_rule" {
  ...
  source_port_range            = "*"
  destination_address_prefixes = concat(local.ipam.networks.test-dev.appgw)
  destination_port_ranges      = concat([local.ports.http, local.ports.https])
  resource_group_name          = "network"
  network_security_group_name  = "dev-australiaeast-infrastructure"
}

resource "azurerm_network_security_rule" "management_dev" {
  name      = "Management-${title(dev)}"
  priority  = 501
  direction = "Inbound"
  access    = "Allow"
  protocol  = "Tcp"
  ...
  source_port_range            = "*"
  destination_address_prefixes = local.ipam.networks.dev.nodes
...
resource "azurerm_network_security_rule" "staff_to_storage_file" {
  ...
  source_port_range            = "*"
  destination_address_prefixes = concat(local.ipam.networks.dev.storage-file-ip)
